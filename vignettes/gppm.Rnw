...
For model specification, the \fct{gppModel} function is used. The \fct{gppModel} always requires 5 arguments: \code{mFormula}, \code{cFormula}, \code{myData}, \code{ID}, and \code{DV}. The last three arguments are trivial. \code{myData} is simply the data frame contain the relevant data in the long-format. \code{ID} is the label of the variable in \code{myData} that contains the subject IDs. \code{DV} is also a variable label and points to the to bo model variable.

The two essential arguments are \code{mFormula} and \code{cFormula}, which specify the mean and the covariance function. The mean and covariance function are specified using strings that describe the respective formulas. The parser formula parser is quite advanced. As an example, the latent growth curve model is specified like this
<<specify LGCM, echo=TRUE>>=
gppModel('muI+muS*t','varI+covIS*(t+t#)+varS*t*t!+(t==t#)*sigma',demoLGCM,'x')
@
The parsers automatically recognizes that \emph{muI} refers to a parameter, \emph{t} to an observed value in the data frame, and \emph{+} to a mathematical operator. Thus, the parameters are defined implicetely via the mean and the covariance functions, and there is no need for specifying them implicetly. For the parser to be guanranteed to work some parameter and variable naming conventions must be adhered too. They can be found in the help file of \fct{gppModel}  and match with common best practices in R. In terms of supported functions and operators all functions that are supported by STAN are supported (see STAN manual part VII) %TODO write help, write tests,
